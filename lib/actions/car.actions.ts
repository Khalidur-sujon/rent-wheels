...
export async function deleteAllCars(): Promise<void> {
	try {
		...
		// Delete all the reviews associated with cars
		...
		// Pull cars from the user's carsAdded and carsRented
		...
		//Finally, delete cars
		const deleteCarResults = await Car.deleteMany({});
		console.log(`Deleted ${deleteCarResults.deletedCount} cars`);
	} catch (error: any) {
		throw new Error(`Failed to delete cars: ${error.message}`);
	}
}
...
export async function fetchCarById(carId: string): Promise<CarParams | null> {
	try {
		...
		throw new Error(`Faile to fetch car by ID: ${error.message}`);
	}
}
...
export async function fetchCarsAddedByUser(
	...
	try {
		...
		throw new Error(
			`Failed to fetch cars added with user by ID: ${error.message}`
		);
	}
}
...
export async function fetchCarsRentedByUser(
	...
	try {
		...
		return user.carsRented.map((rented: any) => {
			...
			if (car.reviews && car.reviews.length > 0) {
				const totalRatings = car.reviews.reduce(
					(acc: number, review: ReviewDocument) =>
						acc + review.rating,
					0
				);
		...
		throw new Error(
			`Failed to fetch rended cars by user with ID: ${error.message}`
		);
	}
}
...
export async function getAllReviewsForCar(
	...
	try {
		...
		throw new Error(
			`Failed to fetch reviews for the car: ${error.message}`
		);
	}
}
